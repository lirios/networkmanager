...
void Networking::activateConnection(const QString &connectionPath, const QString &device, const QString &specificObject)
{
    ...
    if (connection->settings()->connectionType() == NetworkManager::ConnectionSettings::Vpn) {
        ...
        if (vpnSetting) {
            ...
            NMVpnPluginInfo *pluginInfo = nm_vpn_plugin_info_list_find_by_service(plugins, vpnSetting->serviceType().toStdString().c_str());
            if (!pluginInfo) {
                auto notification = new Liri::Notification(this);
                notification->setApplicationName(QLatin1String("networkmanagement"));
                notification->setApplicationIcon(QLatin1String("dialog-warning"));
                notification->setSummary(connection->name());
                notification->setBody(tr("Missing VPN plugin"));
                notification->send();
                return;
            }
        }
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this](QDBusPendingCallWatcher *self) {
        ...
        if (reply.isError() || !reply.isValid()) {
            ...
            notification->setApplicationName(QLatin1String("networkmanagement"));
            notification->setApplicationIcon(QLatin1String("dialog-warning"));
            notification->setSummary(tr("Failed to activate %1").arg(connectionName));
    ...
    if (securityType == NetworkManager::Leap ||
        ...
        connect(watcher, &QDBusPendingCallWatcher::finished, this, [this](QDBusPendingCallWatcher *self) {
            ...
            if (reply.isError() || !reply.isValid()) {
                ...
                notification->setApplicationName(QLatin1String("networkmanagement"));
                notification->setApplicationIcon(QLatin1String("dialog-warning"));
                notification->setSummary(tr("Failed to add %1").arg(connectionName));
                notification->setBody(error);
                notification->send();
            }
...
void Networking::deactivateConnection(const QString &connectionName, const QString &device)
{
    ...
    connect(watcher, &QDBusPendingCallWatcher::finished, this, [this](QDBusPendingCallWatcher *self) {
        ...
        if (reply.isError() || !reply.isValid()) {
            ...
            notification->setApplicationName(QLatin1String("networkmanagement"));
            notification->setApplicationIcon(QLatin1String("dialog-warning"));
            notification->setSummary(tr("Failed to deactivate %1").arg(connectionName));
            notification->setBody(error);
            notification->send();
        }
